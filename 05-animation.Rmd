...
* Animation can be easy to add to ggplot with the gganimate package. I've had a play [here](https://github.com/billster45/gganimate-experiments/blob/master/README.md) inspired by a good Tweet of examples you could adapt.

* I've found animation is most effective to show changes over time. Here is the annual pattern of house sales changing each year.
...
* The code is as simple as specifying which column of categories we animate over as frames.
...
df_red %>% 
  ...
  plotly::add_lines(
    x = ~month, 
    y = ~sales,
    frame = ~year,  # this line does the animation
    color = ~factor(city),

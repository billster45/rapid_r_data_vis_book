...
* But for easy to read tables we usually need to make our long tidy data "wide" with many observations across columns. To create a wide interactive table we use tidyr::pivot_wider() then pipe that wide data into DT::datatable()
...
* Often you have decided on lots of differet [options][https://rstudio.github.io/DT/options.html] that you want apply to many tables. Reduce the amount of by setting your main DT table options globally like this.
...
* I love Excel pivot tables and charts for exploring data.rpivotables are a similar exploratory tool in R. We simply put the data frame into the function rpivotTable::rpviotTable() Then play around with the html output below as you would an Excel pivot table.
...
* Once we've decided how we want the rpivotTable to look manually we can specify the layout that tells our story as follows.
